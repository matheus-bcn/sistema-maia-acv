...
export function BatalhaX1({ rankings }: BatalhaX1Props) {
  ...
  return (
    <div className="glass-card rounded-2xl p-6 border-yellow-500/20 relative overflow-hidden">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 h-20 w-20 bg-yellow-500/20 blur-3xl" />

      <div className="flex items-center justify-between mb-8">
        <h3 className="text-lg font-black uppercase tracking-widest text-yellow-500 flex items-center gap-2">
      ...
      <div className="flex items-center justify-between gap-8 relative">
        <div className="text-left flex-1">
          <div className="h-16 w-16 rounded-full bg-blue-500/20 border-2 border-blue-400 flex items-center justify-center text-xl font-black mb-2">
            {player1.seller.avatar ?? player1.seller.name.charAt(0)}
          </div>
          <p className="font-bold text-lg truncate">{player1.seller.name}</p>
          <p className="text-blue-400 font-black text-xl">R$ {v1.toLocaleString("pt-BR")}</p>
        </div>
        ...
        <div className="text-right flex-1">
          <div className="h-16 w-16 rounded-full bg-red-500/20 border-2 border-red-400 flex items-center justify-center text-xl font-black mb-2 ml-auto">
            {player2.seller.avatar ?? player2.seller.name.charAt(0)}
          </div>
          <p className="font-bold text-lg truncate">{player2.seller.name}</p>
          <p className="text-red-400 font-black text-xl">R$ {v2.toLocaleString("pt-BR")}</p>
        </div>
      </div>
