...
export function PerfilVendedorModal({ isOpen, onClose, sellerStats }: PerfilVendedorModalProps) {
  const ticketMedio = sellerStats && sellerStats.salesCount > 0 
    ...
    <AnimatePresence>
      {isOpen && sellerStats && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          ...
          >
            <div className="absolute top-0 left-0 w-full h-32 bg-gradient-to-br from-purple-500/20 to-transparent -z-10" />

            <button 
              onClick={onClose} 
              className="absolute top-4 right-4 p-2 text-neutral-400 hover:text-white bg-black/20 hover:bg-white/10 rounded-full transition-all z-10"
            >
              <X className="h-5 w-5" />
            </button>

            <div className="flex flex-col items-center mt-2 mb-6">
              <div className="h-20 w-20 rounded-full bg-gradient-to-br from-purple-400 to-purple-600 flex items-center justify-center text-3xl font-black text-black border-4 border-neutral-900 shadow-[0_0_20px_rgba(52,211,153,0.3)] mb-4 uppercase">
                {sellerStats.name.charAt(0) || "V"}
              </div>
              <h3 className="text-2xl font-black tracking-tight">{sellerStats.name}</h3>
              <div className="flex items-center gap-1.5 text-purple-400 mt-2 bg-purple-500/10 px-3 py-1.5 rounded-full border border-purple-500/20">
