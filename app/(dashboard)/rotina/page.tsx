...
interface KanbanCard {
  ...
  position: number; due_date: string | null; priority: string;
}

// ── Helpers de cor e prioridade ────────────────────────────
...
const PRIORITY_CFG: Record<string, { color: string; bg: string; label: string }> = {
  Alta:   { color: "text-red-400",     bg: "bg-red-500/20 border-red-500/40",       label: "Alta"   },
  Normal: { color: "text-blue-400",    bg: "bg-blue-500/20 border-blue-500/40",     label: "Normal" },
  Baixa:  { color: "text-neutral-400", bg: "bg-neutral-700/60 border-neutral-600",  label: "Baixa"  },
};
...
function CardItem({
  card, lists, onEdit, onDelete, onMove,
}: {
  card: KanbanCard; lists: KanbanList[];
            ...
            <div className="flex items-center flex-wrap gap-1.5 mt-2">
              {card.priority !== "Normal" && (
                <span className={cn("flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[10px] font-bold border", pri.bg, pri.color)}>
                  <Icon icon="mdi:flag" className="h-2.5 w-2.5" />
                  {card.priority}
                </span>
              )}
              {card.due_date && (
                <span className={cn(
                  ...
                  overdue
                    ...
                    : "bg-neutral-700/60 border-neutral-600 text-neutral-400"
                )}>
...
function KanbanColumn({
  list, cards, lists, onAddCard, onEditList, onDeleteList, onEditCard, onDeleteCard, onMoveCard,
}: {
  list: KanbanList; cards: KanbanCard[]; lists: KanbanList[];
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={onClose}
            className="p-1.5 hover:bg-white/10 rounded-full text-neutral-500 hover:text-white transition-colors">
...
export default function RotinaPage() {
  ...
  const openModal = (
    ...
    opts?: { listId?: string; card?: KanbanCard; list?: KanbanList }
  ) => {
    setModalError(null);
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (modal.type === "addList") {
        ...
        const res = await criarCartaoAction(modal.listId, inputTitle, inputDesc, inputPriority, inputDueDate, selectedSellerId);
        if (res.error) { setModalError(res.error); return; }
        await carregarDados(selectedSellerId);

      } else if (modal.type === "editCard" && modal.card) {
        if (!inputTitle.trim()) { setModalError("Digite um título."); return; }
        const res = await editarCartaoAction(modal.card.id, inputTitle, inputDesc, inputPriority, inputDueDate);
        if (res.error) { setModalError(res.error); return; }
        await carregarDados(selectedSellerId);
