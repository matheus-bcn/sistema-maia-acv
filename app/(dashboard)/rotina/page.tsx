...
function CardItem({
  card, lists, sellers, canEdit, onEdit, onDelete, onMove,
}: {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div {...(canEdit ? { ...attributes, ...listeners } : {})} className={cn("flex-1 min-w-0", canEdit && "cursor-grab active:cursor-grabbing")}>
          <p className="text-sm font-semibold text-white leading-snug">{card.title}</p>
          {card.description && <p className="text-xs text-neutral-400 mt-1 leading-relaxed line-clamp-2">{card.description}</p>}
...
function KanbanColumn({
  list, cards, lists, sellers, canEdit, onAddCard, onEditList, onDeleteList, onEditCard, onDeleteCard, onMoveCard,
}: {
...
export default function RotinaPage() {
  ...
  // canEdit: admin sempre pode; vendedor apenas no painel geral se habilitado
  const canEdit = isAdmin;

  // targetSellerId para criação (admin no painel vendedor = seller selecionado; demais = undefined)
  ...
  // Quando no modo vendedor e nenhum seller selecionado, não mostra board
      ...
      <header className="mb-4 flex-shrink-0">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {/* Toggle PAINEL GERAL / PAINEL VENDEDOR */}
