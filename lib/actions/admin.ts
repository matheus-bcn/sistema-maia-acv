...
export async function updateMonthlyGoalAction(amount: number) {
  ...
  if (!(await isSellerAdmin(supabase, user.id, user.email))) return { error: "Acesso negado: apenas administradores podem alterar a meta." };
...
export async function createAwardAction(title: string, requiredAmount: number, description: string) {
  ...
  if (!(await isSellerAdmin(supabase, user.id, user.email))) return { error: "Acesso negado: apenas administradores podem criar premiações." };
